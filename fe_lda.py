...
lda.fit(X_scale, y)
lda_obj = lda.transform(X_scale)
lda_obj_df = pd.DataFrame(lda_obj)
print("lda.explained_variance_ratio_", lda.explained_variance_ratio_)
print("sum of lda.explained_variance_ratio_", sum(lda.explained_variance_ratio_))
plt.figure(figsize=(10, 7))
sns.scatterplot(x=lda_obj[:, 0], y=lda_obj[:, 1], s=70, hue=data.winner,
                palette=['steelblue', 'orangered', 'darkslategrey'])
plt.title("Class Distribution with LDA")
plt.xlabel('LD1')
plt.ylabel('LD2')
plt.show()
...
print("LDA results START")
run_with_k_fold_techniques(lda_obj_df.values, y)
run_with_train_test_split_techniques(lda_obj_df,y)
print("LDA results FINISH")
